...
| Add Task          | Eric  | Fill title, description, due date, priority, type; save to tasks table.                     | Cancel/ go Back                     |
| View Tasks        | Eric  | Query tasks for logged-in user; display in TableView.                                       | Empty list; add a task              |
| Edit Task         | Loi   | Select task, edit fields, save; update DB record.                                           | Cancel, Delete, no DB changes.      |
| Mark Complete     | Loi   | Click Mark Complete; set status = Completed “1 or 0” in DB; refresh list.                   | Already complete: Reopen option.    |
...
| task_types | typeId (PK), typeName, isEnabled                                                  | Global availability        |
...
| User Dashboard  | Display tasks count of each priority, total tasks, Add Task button, View Tasks button, Log Out button                              | Loi T    |
...
| Edit Task       | Pre-filled fields for title, description, due date, priority, status. Save, Mark Complete checkbox, Cancel buttons, Delete Button. | Loi T    |
...
| Loi    | User Dashboard, Edit Task, Mark Complete, Delete task. | Create tasks table; Build Edit task scene, Mark complete; Delete task. | name/user-dashboard, name/edit-task, name/task-status-test               |
| Eric   | Task List, Add Task, External API                      | Build task list ;Add task scene; unit tests                            | name/add-task-scene, name/add-task-list, name/task-unit-test/create-test |
...
## Scene / User Flow
![Scene Diagram](/sceneDiagram.png "Scene Diagram")
